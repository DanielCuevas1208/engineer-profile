...
import type { DeployAdapterName, DeploymentConfig } from "../types.js";
...
export interface DeploymentAdapter {
  readonly name: DeployAdapterName;
...
export function resolveAdapter(name: string): DeploymentAdapter {
  ...
  if (!adapter) {
    throw new Error(
      `Unknown deployment adapter "${name}". Available adapters: ${listAdapters().join(", ")}.`
    );
  }
  return adapter;
}
