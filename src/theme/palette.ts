import { DEFAULT_THEME_NAME, type ThemeConfig } from "../types.js";
...
export function resolveTheme(config?: ThemeConfig): ThemeTokens {
  const requestedName = config?.name ?? DEFAULT_THEME_NAME;
  const baseName = isBuiltinTheme(requestedName) ? requestedName : DEFAULT_THEME_NAME;
  const base = BUILTIN_PALETTES[baseName];
  const tokens: ThemeTokens = { ...base, name: baseName };

  if (config?.accent && isValidHexColor(config.accent)) {
