...
addConfigOption(program
  ...
  .action(async (options) => {
    ...
    const deployment = applyDeployAdapter(config);
    ...
    console.log(`Prepared deployment for ${deployment.adapter}${deployment.domain ? ` (${deployment.domain})` : ""}.`);
...
program
  ...
  .action(() => {
    for (const theme of listThemes()) {
      console.log(`${theme.name} - ${theme.label} (${theme.colorScheme})`);
    }
  });
...
addConfigOption(program
  ...
  .option("-a, --adapter <name>", "Deployment adapter name")
