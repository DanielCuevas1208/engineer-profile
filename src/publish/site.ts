...
.tag { padding: 5px 9px; border: 1px solid var(--tag-border); border-radius: 999px; background: var(--tag-bg); color: var(--mint); font: 0.68rem/1 "SFMono-Regular", Consolas, monospace; }
...
.button.primary:hover { background: var(--blue-soft); color: var(--button-text); }
...
export function publishSite(config: PortfolioConfig): PublishResult {
  ...
  try {
    ...
    const themeLabel = `Theme: ${escapeHtml(theme.name)}`;
  ...
  <div class="site-shell">
    <div class="container">
      <nav class="site-nav" aria-label="Primary navigation">
        <a class="brand" href="#top"><span class="brand-mark">${escapeHtml(brandMark(config.title))}</span><span>${escapeHtml(config.title)}</span></a>
      ...
      <section class="audit-panel" aria-label="Data audit status">
        ...
        <div class="audit-time">Generated ${escapeHtml(generatedAt.slice(0, 19).replace("T", " "))} UTC<br />Emails redacted / hidden projects excluded<br />${themeLabel}</div>
      </section>
      <main>
    ...
    const screenshotsCopied = copyScreenshots(config);
    const manifestPath = join(config.outputDir, "site-manifest.json");
    const publishedFiles = listFiles(config.outputDir);
    const manifest: SiteManifest = {
      ...
      projects: views.map((view) => ({
        ...
        stars: view.project.stars,
        commits: view.evidence.commits,
        releases: view.evidence.releases,
        visible: true,
        hasScreenshot: view.project.screenshot_path !== null,
        changelogFile: view.changelog.length > 0 ? `${view.project.slug}-changelog.md` : null,
      })),
      files: [...publishedFiles, "site-manifest.json"].sort(),
    ...
    return { indexPath, manifestPath, projectCount: projects.length, generatedAt, theme: theme.name, screenshotsCopied };
  } finally {
    db.close();
  }
}

function copyScreenshots(config: PortfolioConfig): number {
