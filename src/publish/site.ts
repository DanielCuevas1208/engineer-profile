...
import { cssVariables, resolveTheme, type Theme } from "../theme/registry.js";
...
a { color: inherit; }
.site-shell { min-height: 100vh; background: radial-gradient(circle at 82% -10%, var(--glow), transparent 34rem), var(--bg); }
...
.brand { display: inline-flex; align-items: center; gap: 12px; font: 700 0.9rem/1 var(--mono); letter-spacing: 0.08em; text-decoration: none; text-transform: uppercase; }
.brand-mark { display: grid; width: 30px; height: 30px; place-items: center; border: 1px solid var(--accent); color: var(--accent); border-radius: 8px; font-size: 0.72rem; }
.nav-link { color: var(--muted); font: 0.76rem/1 var(--mono); letter-spacing: 0.08em; text-decoration: none; text-transform: uppercase; }
.nav-link:hover, .text-link:hover, h2 a:hover { color: var(--accent); }
.hero { display: grid; grid-template-columns: minmax(0, 1.3fr) minmax(300px, 0.7fr); gap: 80px; align-items: end; padding: 86px 0 70px; }
.kicker, .eyebrow, .visual-label, .card-topline, .section-heading, .source-badge, .audit-label { font: 0.7rem/1.4 var(--mono); letter-spacing: 0.12em; text-transform: uppercase; }
.kicker { color: var(--accent-2); margin: 0 0 22px; }
h1 { max-width: 760px; margin: 0; font-size: clamp(3.2rem, 8vw, 6.4rem); font-weight: 650; letter-spacing: -0.08em; line-height: 0.94; }
.hero-copy { max-width: 530px; margin: 28px 0 0; color: var(--accent-soft); font-size: 1.12rem; }
.hero-aside { padding: 22px; border: 1px solid var(--line); border-radius: 14px; background: linear-gradient(145deg, var(--panel-a), var(--panel-b)); box-shadow: var(--shadow); }
.aside-index { display: flex; justify-content: space-between; color: var(--accent-3); font: 0.68rem/1 var(--mono); letter-spacing: 0.12em; text-transform: uppercase; }
...
.stat span { color: var(--muted); font: 0.7rem/1.3 var(--mono); letter-spacing: 0.08em; text-transform: uppercase; }
.audit-panel { display: grid; grid-template-columns: 1fr auto; gap: 20px; align-items: center; margin: 26px 0 80px; padding: 18px 20px; border: 1px solid var(--line); border-radius: 10px; background: var(--panel-tint); }
.audit-copy { color: var(--muted); font-size: 0.88rem; }
.audit-copy strong { color: var(--text); font-weight: 500; }
.audit-time { color: var(--accent); font: 0.7rem/1.4 var(--mono); text-align: right; }
...
.project-card { display: grid; grid-template-columns: minmax(280px, 0.8fr) minmax(0, 1.2fr); overflow: hidden; margin-bottom: 24px; border: 1px solid var(--line); border-radius: 16px; background: linear-gradient(135deg, var(--panel-a), var(--panel-b)); box-shadow: var(--shadow); }
...
.placeholder { display: flex; min-height: 310px; align-items: center; justify-content: center; flex-direction: column; gap: 7px; color: var(--accent); background: repeating-linear-gradient(135deg, var(--stripe), var(--stripe) 1px, transparent 1px, transparent 14px); }
.placeholder small { color: var(--muted); font: 0.68rem/1 var(--mono); text-transform: uppercase; }
.visual-label { position: absolute; right: 16px; bottom: 16px; padding: 7px 9px; border: 1px solid rgba(255,255,255,0.18); border-radius: 5px; background: rgba(8, 17, 31, 0.72); color: var(--accent-soft); }
.project-body { padding: 30px 34px 32px; }
.card-topline { display: flex; justify-content: space-between; gap: 12px; color: var(--accent); }
...
.description { max-width: 620px; margin: 0; color: var(--accent-soft); font-size: 0.98rem; }
.facts { display: flex; flex-wrap: wrap; gap: 22px; margin: 24px 0 16px; color: var(--muted); font: 0.76rem/1 var(--mono); }
.facts strong { color: var(--text); font-size: 1rem; font-weight: 600; }
.tags { display: flex; flex-wrap: wrap; gap: 7px; margin-bottom: 26px; }
.tag { padding: 5px 9px; border: 1px solid var(--tag-line); border-radius: 999px; color: var(--accent-2); font: 0.68rem/1 var(--mono); }
.change-log { padding: 18px 0 20px; border-top: 1px solid var(--line); border-bottom: 1px solid var(--line); }
.section-heading { display: flex; justify-content: space-between; color: var(--accent-3); }
.source-badge { color: var(--muted); }
.change-log h3 { margin: 12px 0 2px; font-size: 1.08rem; font-weight: 550; }
.change-date { margin: 0 0 10px; color: var(--muted); font: 0.7rem/1.3 var(--mono); }
...
.change-preview code { padding: 2px 4px; border-radius: 3px; color: var(--accent-2); background: var(--code-bg); font: 0.76rem var(--mono); }
.text-link { display: inline-block; margin-top: 14px; color: var(--accent); font: 0.73rem/1 var(--mono); text-decoration: none; text-transform: uppercase; }
.card-actions { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 22px; }
.button { display: inline-block; padding: 10px 14px; border-radius: 7px; font: 0.72rem/1 var(--mono); letter-spacing: 0.04em; text-decoration: none; text-transform: uppercase; }
...
.audit-list li { display: grid; grid-template-columns: 150px 72px 1fr; gap: 12px; padding: 8px 0; border-bottom: 1px solid var(--line-soft); color: var(--muted); font: 0.72rem/1.4 var(--mono); }
.audit-list time { color: var(--accent); }
.audit-list strong { color: var(--accent-3); font-weight: 500; text-transform: uppercase; }
.site-footer { display: flex; justify-content: space-between; gap: 20px; padding: 24px 0 40px; border-top: 1px solid var(--line); color: var(--muted); font: 0.7rem/1.4 var(--mono); }
...
<body>
  <div class="site-shell">
    ...
    return { indexPath, projectCount: projects.length, generatedAt, theme: theme.name };
  } finally {
    db.close();
  }
}
