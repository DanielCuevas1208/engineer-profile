...
import { getTheme, type ThemeDefinition } from "../theme/registry.js";
...
function themeCss(theme: ThemeDefinition): string {
...
.aside-index { display: flex; justify-content: space-between; color: var(--accent-3); font: 0.68rem/1 var(--font-mono); letter-spacing: 0.12em; text-transform: uppercase; }
...
.audit-panel { display: grid; grid-template-columns: 1fr auto; gap: 20px; align-items: center; margin: 26px 0 80px; padding: 18px 20px; border: 1px solid var(--line); border-radius: 10px; background: color-mix(in srgb, var(--panel) 70%, transparent); }
.audit-copy { color: var(--muted); font-size: 0.88rem; }
.audit-copy strong { color: var(--text); font-weight: 500; }
.audit-time { color: var(--accent); font: 0.7rem/1.4 var(--font-mono); text-align: right; }
...
.project-card { display: grid; grid-template-columns: minmax(280px, 0.8fr) minmax(0, 1.2fr); overflow: hidden; margin-bottom: 24px; border: 1px solid var(--line); border-radius: 16px; background: linear-gradient(135deg, var(--panel-strong), var(--ink-soft)); box-shadow: var(--shadow); }
...
.placeholder { display: flex; min-height: 310px; align-items: center; justify-content: center; flex-direction: column; gap: 7px; color: var(--accent); background: repeating-linear-gradient(135deg, color-mix(in srgb, var(--accent) 8%, transparent), color-mix(in srgb, var(--accent) 8%, transparent) 1px, transparent 1px, transparent 14px); }
.placeholder small { color: var(--muted); font: 0.68rem/1 var(--font-mono); text-transform: uppercase; }
.visual-label { position: absolute; right: 16px; bottom: 16px; padding: 7px 9px; border: 1px solid var(--line); border-radius: 5px; background: color-mix(in srgb, var(--ink) 72%, transparent); color: var(--accent-soft); }
...
.tag { padding: 5px 9px; border: 1px solid color-mix(in srgb, var(--accent-2) 26%, transparent); border-radius: 999px; color: var(--accent-2); font: 0.68rem/1 var(--font-mono); }
...
.change-preview code { padding: 2px 4px; border-radius: 3px; color: var(--accent-2); background: color-mix(in srgb, var(--accent-2) 8%, transparent); font: 0.76rem var(--font-mono); }
.text-link { display: inline-block; margin-top: 14px; color: var(--accent); font: 0.73rem/1 var(--font-mono); text-decoration: none; text-transform: uppercase; }
...
.audit-list li { display: grid; grid-template-columns: 150px 72px 1fr; gap: 12px; padding: 8px 0; border-bottom: 1px solid color-mix(in srgb, var(--line) 55%, transparent); color: var(--muted); font: 0.72rem/1.4 var(--font-mono); }
...
<head>
  ...
  <meta name="theme-color" content="${theme.tokens.ink}" />
  ...
  <style>${themeCss(theme)}${siteCss()}</style>
</head>
<body>
