...
function siteCss(tokens: ThemeTokens): string {
  return `
...
export const SITE_MANIFEST_NAME = "site-manifest.json";
const SITE_MANIFEST_VERSION = 1;
...
function listOutputFiles(outputDir: string): string[] {
  ...
  const walk = (current: string): void => {
    for (const entry of readdirSync(current, { withFileTypes: true })) {
      ...
      else files.push(relative(outputDir, join(current, entry.name)).split("\\").join("/"));
    }
  };
  walk(outputDir);
  return files.sort();
...
export function writeSiteManifest(
  config: PortfolioConfig,
  snapshot: { generatedAt: string; theme: string; projectCount: number }
): string {
  const manifestPath = join(config.outputDir, SITE_MANIFEST_NAME);
  const manifest: SiteManifest = {
    formatVersion: SITE_MANIFEST_VERSION,
    generatedAt: snapshot.generatedAt,
    owner: config.owner,
    theme: snapshot.theme,
    projectCount: snapshot.projectCount,
    files: listOutputFiles(config.outputDir),
  };
...
<body>
  <div class="site-shell">
    ...
    const manifestPath = writeSiteManifest(config, {
      generatedAt,
      theme: tokens.name,
    ...
    return { indexPath, manifestPath, theme: tokens.name, projectCount: projects.length, generatedAt };
  } finally {
    db.close();
  }
}
...
export function copyScreenshotsToOutput(config: PortfolioConfig): number {
  ...
  try {
    ...
    const visible = db.listProjects(true).length;
    const tokens = resolveTheme(config.theme);
    writeSiteManifest(config, {
      generatedAt: config.clock(),
      theme: tokens.name,
