...
describe("portfolio configuration", () => {
  ...
  it("rejects an empty theme name", () => {
    mkdirSync(TEST_DIR, { recursive: true });
    writeFileSync(TEST_FILE, JSON.stringify({ theme: "  " }));

    expect(() => loadPortfolioConfig(TEST_FILE)).toThrow(
      'Configuration field "theme" must be a non-empty string.'
    );
  });
  ...
  it("rejects a malformed deployment target directory", () => {
    mkdirSync(TEST_DIR, { recursive: true });
    writeFileSync(TEST_FILE, JSON.stringify({ deploy: { adapter: "local", targetDir: "" } }));

    expect(() => loadPortfolioConfig(TEST_FILE)).toThrow(
      'Configuration field "deploy.targetDir" must be a non-empty string.'
    );
  });
});
